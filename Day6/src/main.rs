...
impl Direction {
    ...
    fn move_forward(self, position: (usize, usize)) -> Option<(usize, usize)> {
        match self {
            Direction::Up => if position.0 == 0 { None } else {Some((position.0 - 1, position.1))},
            Direction::Down => Some((position.0 + 1, position.1)),
            Direction::Left => if position.1 == 0 { None } else {Some((position.0, position.1 - 1))},
...
fn main() {
    ...
    if !solve_part2 {
        ...
        loop {
            if let Some(next_position) =  start_direction.move_forward(start_position) {
        ...
        let simulate = |map: &Vec<Vec<char>>, obstruction: Option<(usize, usize)>| -> bool {
            ...
            loop {
                ...
                if let Some(next_position) = direction.move_forward(position) {
                    ...
                    if map[next_position.0][next_position.1] == '#' || obstruction == Some(next_position) {
