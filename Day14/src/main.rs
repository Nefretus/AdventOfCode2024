...
fn calculate_safety_factor(mut robots: Vec<Robot>, width: isize, height: isize, steps: usize) -> (usize, usize) {
    for _ in 0..steps {
...
fn render_grid(robots: &Vec<Robot>, width: usize, height: usize) {
    ...
    for row in grid {
        println!(
            "{}",
            row.iter()
                .map(|&count| if count == 0 { '.' } else { char::from_digit(count as u32, 10).unwrap_or('9') })
                .collect::<String>()
        );
    }
    println!();
...
fn main() -> io::Result<()>{
    ...
    let iterations = 100;
    let (safety_factor, christmas_tree_event) = calculate_safety_factor(robots, width as isize, height as isize, iterations);
    println!("Safety factor: {} Christams tree after: {} iterations", safety_factor, christmas_tree_event);
